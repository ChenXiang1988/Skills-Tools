---
name: pm-prd
description: Create right-sized product requirements documents. Use when Codex needs to write, review, resize, or refactor PRDs using L1/L2/L3 depth, requirement-analysis inputs, metrics definitions, scenario catalogs, use cases, artifact plans, field dictionaries, acceptance criteria, and standalone diagram references.
---
...
Do not perform requirement analysis, invent metrics, force technical stack selection, create HTML prototypes, generate review artifacts, or embed full diagrams in the PRD body.
...
Stop when the problem statement is actually a solution proposal, when only the happy path exists, or when acceptance criteria are subjective and cannot be tested.
...
- Wrong use: asking for frontend, backend, database, or deployment choices before technical constraints are proven to affect the product requirement.
- Bad output: the problem statement is a UI control, feature idea, or implementation proposal.
- Bad output: normal flow exists but empty, loading, error, permission, timeout, duplicate-action, or concurrent-state behavior is missing when relevant.
...
- Bad output: acceptance criteria are subjective, such as "beautiful", "simple", or "easy to use", and cannot be converted into test scenarios.
- Bad output: same-level requirements have inconsistent granularity.
- Bad output: assumptions, open questions, or source conflicts are written as facts.
- Bad output: diagrams are embedded in the PRD body when standalone diagram files are required by the artifact plan or workspace standard.
