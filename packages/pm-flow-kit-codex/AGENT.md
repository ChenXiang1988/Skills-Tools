...
- Treat local plugin modification and Codex plugin update as two separate actions.
- When the human asks to modify, optimize, fix, or test `pm-flow-kit`, default to local files only.
- Do not sync changes to Codex skill directories, do not reinstall the plugin, do not run `codex plugin add`, and do not update the plugin cachebuster unless the human explicitly asks to update Codex.
- Phrases that count as explicit Codex update approval include: "同步到 Codex", "更新到 Codex", "重新安装插件", "让 Codex 使用新版", or equivalent wording.
- After local validation, report the local result and wait for the human's explicit instruction before updating Codex.
