...
> 本章承接需求分析文档 §6（目标系统边界），仅补充需求分析阶段未覆盖的内容，已确认项不再重复填写。
> 如需求分析阶段已明确以下内容，在本章直接引用结论即可，无需重新填写：
>
> - 目标系统：`OMS` / `WMS` / `TMS` / 其他（来自需求分析 §6.1）
> - 库存责任归属（来自需求分析 §6.2）
> - 作业节点/状态口径（来自需求分析 §6.3）
> - 是否手工单（来自需求分析 §6.4）
> - 与物流商揽收系统/TMS 的交接口径（来自需求分析 §6.5）
> - 不应单列为业务场景的状态/节点（来自需求分析 §6.6）
>
> 以下两项为 Plan 阶段新增，需求分析阶段不涉及，必须填写：
