...
> 本文件为 **WorkBuddy 适配版**（去 Codex 化）。相对上游 `pm-flow-kit-codex`，本套件额外内置 `design-system/` 设计系统，其余治理层（Agent Protocol、工作区/知识库绑定门禁、阶段专项 skill）保持一致。
...
本套件内置 `design-system/`（Web = Ant Design v5；PDA = antd-mobile 冻结快照），是 `pm-html-prototype` 的默认 token 源：脚手架会把 `tokens.css` 与 `.yc-*` 组件快照进每个原型。其它设计语言按用户提供的规范执行。这是本套件相对上游 `pm-flow-kit-codex` 的独有优势，上游不包含设计系统。
